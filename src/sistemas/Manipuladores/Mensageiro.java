...
public class Mensageiro extends MultiPartEmail {
	...
	public boolean enviarHistoricoDeCorridas(Usuario p) {
		...
		try {
			central = (CentralDeInformacoes) per.recuperar("src/arquivos/dados-passageiros.xml");
		...
		try {
			setMsg(String.format(
					"Olá %s, em anexo está o seu histórico de corridas no nosso aplicativo! \n\n\nMuito obrigado pela preferência!\nAtenciosamente, \nEquipe Monteiro Motos",
					p.getNome()));

			EmailAttachment anexo = new EmailAttachment();
			anexo.setPath("src/arquivos/relatorio.pdf");
			...
			send();
			JOptionPane.showMessageDialog(null, "Histórico enviado para o email");
			return true;
	...
	public boolean enviarCodigoDeRecuperacao(String email, String codigoChave, String titulo) {
		try {
			...
			send();
			JOptionPane.showMessageDialog(null, "O código de recuperação foi enviado para o email");
			return true;
		} catch (EmailException e) {
	...
	public boolean enviarQueCorridaFoiConcluida(String email, Corrida corrida) {
		try {
			...
			addTo(email);
			setMsg("A corrida de id: " + corrida.getId()
	...
	public boolean enviarQueCorridaFoiReinvindicada(String email, Corrida corrida, Mototaxista m) {
		try {
			...
			send();
			JOptionPane.showMessageDialog(null, "O email de conclusão da corrida foi enviado");
			return true;
		} catch (EmailException e) {
	...
	public boolean enviarRelatorioFinancas(String email) {
		try {
			...
			addTo(email);
			EmailAttachment anexo = new EmailAttachment();
			anexo.setPath("src/arquivos/Relatorio_de_Finanças.pdf");
	...
	public boolean enviarBoleto(String email) {
		...
		try {
			addTo(email);
			EmailAttachment anexo = new EmailAttachment();
			anexo.setPath("src/arquivos/Boleto.pdf");
