...
public class GeradorDePDF {
	...
	public void gerarBoleto(CentralDeInformacoes central, Mototaxista m, float valor) {
		...
		String arquivoPDF = "src/arquivos/Boleto.pdf";
		...
		try {
			...
			PdfPCell celula5 = new PdfPCell(new Paragraph(
			...
			PdfPCell celula9 = new PdfPCell(
					new Paragraph(String.format("Pagador:\n\n%s\n%s\n ", m.getNome().toUpperCase(), m.getEmail())));
	...
	public void gerarRelatorioFinancas(ArrayList<Mototaxista> mototaxistas) {
		...
		String arquivoPDF = "src/arquivos/Relatorio_de_Finanças.pdf";
