			...
			if (e.getActionCommand().equals("Confirmar")) {
				...
				if (esc == 0 && corrida.getStatus() == Status.PENDENTE && m.getCorridaAtual() == null) {
					try {
						...
						JOptionPane.showMessageDialog(janela, "Corrida Reivindicada!", "Reivindicada!",
						...
						txData.setText(String.format("Data: %s/%s/%s    Hora: %d:%d", corrida.getData().getDayOfMonth(),
								corrida.getData().getMonthValue(), corrida.getData().getYear(),
								corrida.getData().getHour(), corrida.getData().getMinute()));
					...
					JOptionPane.showMessageDialog(janela, "Esta corrida já foi reinvindicada", "Erro!",
							JOptionPane.ERROR_MESSAGE);
				} else if (m.getCorridaAtual() != null) {
					JOptionPane.showMessageDialog(janela, "Você não pode reinvindicar mais que uma corrida por vez!",
